...
namespace FleetPulse.Tests.UnitTests
{
    public class CompressorTests
    {
        ...
        private static List<GpsPingDto> GetHighSpeedPings()
        {
        ...
        {
            // Arrange
            var pings = new List<GpsPingDto>
            {
                new GpsPingDto { Id = 1, Status = "moving" , Speed=50 },
                new GpsPingDto { Id = 2, Status = "stopped", Speed=0 },
            };
            var compressionService = CreateCompressionInstance();
            // Act
        ...
        public async Task DropIntermediateStoppedPings_3_items_ShouldCompressStoppedPings()
        {
            ...
            var pings = new List<GpsPingDto>
            {
                new GpsPingDto { Id = 1, Status = "moving" , Speed=50 },
                new GpsPingDto { Id = 2, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 3, Status = "stopped", Speed=0 },
            };
            var compressionService = CreateCompressionInstance();
            // Act
            var compressedPings = await compressionService.DropIntermediateStoppedPings(pings);
        ...
        public async Task DropIntermediateStoppedPings_ShouldCompressStoppedPings()
        {
            ...
            var pings = new List<GpsPingDto>
            {
                ...
                new GpsPingDto { Id = 2, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 3, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 4, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 5, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 6, Status = "moving", Speed=50 },
                new GpsPingDto { Id = 7, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 8, Status = "stopped", Speed=0 },
                new GpsPingDto { Id = 9, Status = "moving", Speed=50 }
            };
            var compressionService = CreateCompressionInstance();
            // Act
            var compressedPings = await compressionService.DropIntermediateStoppedPings(pings);
        ...
        public async Task DropIntermediateStoppedPingsLinq_ShouldCompressStoppedPings()
        {
            ...
            public void StoppedConditional_ReturnsTrue_ForStoppedStatus()
            {
                ...
                service.StoppedConditional(new GpsPingDto { Status = "stopped", Speed = 0 }).Should().BeTrue();
            }
            ...
            {
                var service = CreateCompressionInstance();
                service.StoppedConditional(new GpsPingDto { Status = "idle", Speed = 5 }).Should().BeTrue();
            }
            ...
            {
                var service = CreateCompressionInstance();
                service.StoppedConditional(new GpsPingDto { Status = "parked", Speed = 5 }).Should().BeTrue();
            }
            ...
            {
                var service = CreateCompressionInstance();
                service.StoppedConditional(new GpsPingDto { Status = "moving", Speed = 0.5 }).Should().BeTrue();
            }
            ...
            {
                var service = CreateCompressionInstance();
                service.StoppedConditional(new GpsPingDto { Status = "moving", Speed = 50 }).Should().BeFalse();
            }
            ...
            {
                var service = CreateCompressionInstance();
                var result = await service.DropIntermediateStoppedPings(new List<GpsPingDto>());
            ...
            public async Task DropIntermediateStoppedPings_AllMoving_ReturnsAllPings()
            {
                ...
                {
                    new GpsPingDto { Id = 1, Status = "moving", Speed = 50 },
                    new GpsPingDto { Id = 2, Status = "moving", Speed = 60 },
                    new GpsPingDto { Id = 3, Status = "moving", Speed = 55 },
                };
                var service = CreateCompressionInstance();
            ...
            public async Task DropIntermediateStoppedPings_AllStopped_KeepsFirstAndLast()
            {
                var pings = new List<GpsPingDto>
                {
                    new GpsPingDto { Id = 1, Status = "stopped", Speed = 0 },
                    new GpsPingDto { Id = 2, Status = "stopped", Speed = 0 },
                    new GpsPingDto { Id = 3, Status = "stopped", Speed = 0 },
                    new GpsPingDto { Id = 4, Status = "stopped", Speed = 0 },
                };
            ...
            {
                var service = CreateCompressionInstance();
                var result = await service.DropIntermediateStoppedPingsLinq(new List<GpsPingDto>());
                result.Should().BeEmpty();
            }
            ...
            {
                var service = CreateCompressionInstance();
                var result = await service.ApplyHighSpeedCompression(new List<GpsPingDto>());
                result.Should().BeEmpty();
            }
            ...
            public async Task ApplyHighSpeedCompression_AllNormalSpeed_ReturnsAllPings()
            {
                var pings = new List<GpsPingDto>
                {
                    new GpsPingDto { Id = 1, Speed = 20 },
                    new GpsPingDto { Id = 2, Speed = 30 },
                    new GpsPingDto { Id = 3, Speed = 40 }, // equal to threshold — kept
                };
                var service = CreateCompressionInstance();
            ...
            public async Task ApplyHighSpeedCompression_AlwaysKeepsFirstAndLastPings()
            {
                ...
                var pings = new List<GpsPingDto>
                {
                    new GpsPingDto { Id = 1, Speed = 80, Timestamp = baseTime },
                    new GpsPingDto { Id = 2, Speed = 80, Timestamp = baseTime.AddSeconds(1) },
                    new GpsPingDto { Id = 3, Speed = 80, Timestamp = baseTime.AddSeconds(2) },
                    new GpsPingDto { Id = 4, Speed = 80, Timestamp = baseTime.AddSeconds(3) },
                    new GpsPingDto { Id = 5, Speed = 80, Timestamp = baseTime.AddSeconds(4) },
                };
                var service = CreateCompressionInstance();
                var result = await service.ApplyHighSpeedCompression(pings);
            ...
            public async Task ApplyHighSpeedCompression_RespectsCustomThreshold()
            {
                ...
                var pings = new List<GpsPingDto>
                {
                    new GpsPingDto { Id = 1, Speed = 50, Timestamp = baseTime },
                    new GpsPingDto { Id = 2, Speed = 50, Timestamp = baseTime.AddSeconds(1) },
                    new GpsPingDto { Id = 3, Speed = 50, Timestamp = baseTime.AddSeconds(2) },
                };
            ...
            {
                var service = CreateCompressionInstance();
                var result = await service.ApplyTemporalCompression(new List<GpsPingDto>());
                result.Should().BeEmpty();
            }
            ...
            public async Task ApplyTemporalCompression_AllMovingNormalSpeed_ReturnsAllPings()
            {
                var pings = new List<GpsPingDto>
                {
                    new GpsPingDto { Id = 1, Status = "moving", Speed = 30 },
                    new GpsPingDto { Id = 2, Status = "moving", Speed = 35 },
                    new GpsPingDto { Id = 3, Status = "moving", Speed = 40 },
                };
                var service = CreateCompressionInstance();
