...
| :--- | :--- | :--- | :--- |
| `fleetpulse_ai_pings_received_total` | Counter | none | Incremented in `KafkaConsumer.consume()` when a message is received from Kafka. |
| `fleetpulse_ai_pings_processed_total` | Counter | `anomaly_detected` (`true`/`false`) | Incremented in `create_ai_worker_handler()` for processed pings and anomaly outcomes. |
| `fleetpulse_ai_anomaly_detection_seconds` | Histogram | `anomaly_type` | Wraps LLM-based anomaly analysis timing in `create_ai_worker_handler()`. |
...
| `fleetpulse_ai_kafka_lag_messages` | Gauge | none | Declared for consumer lag tracking in `prometheus.py` (placeholder for lag updates). |
...
| `signalr-worker` | `signalr-worker:8080` | `/metrics` |
